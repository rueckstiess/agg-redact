...
describe("redact", function () {
  context("scheme 'typed'", function () {
    ...
    it("retains the type when redacting dates", function () {
      const d = new Date();
      expect(redact(d, { scheme: "typed" })).to.be.equal("date");
    });
    it("retains the type when redacting numbers", function () {
      const n = 1234.56;
      expect(redact(n, { scheme: "typed" })).to.be.equal("number");
    });
    it("redacts different dates the same", function () {
      const date1 = new Date("2021-05-31T11:22:33.444Z");
      const date2 = new Date("2020-05-31T11:22:33.444Z");
      expect(redact(date1, { scheme: "typed" })).to.be.deep.equal(
        redact(date2, { scheme: "typed" })
      );
    });
    it("redacts arrays of strings", function () {
      const input = ["food", "bike", "cloud"];
      const output = redact(input, { scheme: "typed" });
      expect(output).to.be.deep.equal(["string", "string", "string"]);
    });
    ...
    it("hashes keys and values in objects", function () {
      expect(redact({ cloud: "bike" }, { scheme: "typed" })).to.be.deep.equal({
        key: "string",
      });
    });

    it("does not hash _id keys in objects", function () {
      expect(redact({ _id: "food" }, { scheme: "typed" })).to.be.deep.equal({
        _id: "string",
      });
    });

    it("hashes _id as a value", function () {
      expect(redact({ cloud: "_id" }, { scheme: "typed" })).to.be.deep.equal({
        key: redact("_id"),
      });
    });

    it("does not preserve dot notation in keys", function () {
      expect(
        redact({ "bike.cloud": "food" }, { scheme: "typed" })
      ).to.be.deep.equal({
        key: "string",
      });
    });
  ...
  context("scheme 'hashed'", function () {
    it("redacts strings", function () {
